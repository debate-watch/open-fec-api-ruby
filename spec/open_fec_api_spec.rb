RSpec.describe OpenFecApi do
  let(:api_key_missing_response){
    {"error"=>{"code"=>"API_KEY_MISSING", "message"=>"No api_key was supplied. Get one at <a href=\"https://api.data.gov/signup/\">https://api.data.gov/signup/</a>"}}
  }
  let(:api_key){
    ENV["OPEN_FEC_API_KEY"]
  }
  let(:candidates_response){
    {"pagination"=>{"pages"=>1020, "count"=>20393, "per_page"=>20, "page"=>1},
     "api_version"=>"1.0",
     "results"=>
      [{"candidate_status"=>"N",
        "active_through"=>2004,
        "incumbent_challenge_full"=>"Unknown",
        "district"=>nil,
        "election_years"=>[2004],
        "party"=>"IND",
        "office"=>"P",
        "candidate_status_full"=>"Not yet a statutory candidate",
        "state"=>"US",
        "incumbent_challenge"=>nil,
        "party_full"=>"Independent",
        "office_full"=>"President",
        "candidate_id"=>"P40002172",
        "name"=>"AABBATTE, MICHAEL THOMAS WITORT",
        "cycles"=>[2004]},
       {"candidate_status"=>"C",
        "active_through"=>2012,
        "incumbent_challenge_full"=>"Challenger",
        "district"=>nil,
        "election_years"=>[2012],
        "party"=>"REP",
        "office"=>"S",
        "candidate_status_full"=>"Statutory candidate",
        "state"=>"UT",
        "incumbent_challenge"=>"C",
        "party_full"=>"Republican Party",
        "office_full"=>"Senate",
        "candidate_id"=>"S2UT00229",
        "name"=>"AALDERS, TIMOTHY NOEL",
        "cycles"=>[2012]},
       {"candidate_status"=>"C",
        "active_through"=>2012,
        "incumbent_challenge_full"=>"Challenger",
        "district"=>"01",
        "election_years"=>[2012],
        "party"=>"REP",
        "office"=>"H",
        "candidate_status_full"=>"Statutory candidate",
        "state"=>"CA",
        "incumbent_challenge"=>"C",
        "party_full"=>"Republican Party",
        "office_full"=>"House",
        "candidate_id"=>"H2CA01110",
        "name"=>"AANESTAD, SAMUEL",
        "cycles"=>[2012]},
       {"candidate_status"=>"N",
        "active_through"=>2008,
        "incumbent_challenge_full"=>"Open seat",
        "district"=>nil,
        "election_years"=>[2008],
        "party"=>"DEM",
        "office"=>"P",
        "candidate_status_full"=>"Not yet a statutory candidate",
        "state"=>"US",
        "incumbent_challenge"=>"O",
        "party_full"=>"Democratic Party",
        "office_full"=>"President",
        "candidate_id"=>"P80002926",
        "name"=>"AARON, LAURA DAVIS",
        "cycles"=>[2008]},
       {"candidate_status"=>"P",
        "active_through"=>2000,
        "incumbent_challenge_full"=>"Unknown",
        "district"=>"01",
        "election_years"=>[2000],
        "party"=>"REP",
        "office"=>"H",
        "candidate_status_full"=>"Statutory candidate in a prior cycle",
        "state"=>"MA",
        "incumbent_challenge"=>nil,
        "party_full"=>"Republican Party",
        "office_full"=>"House",
        "candidate_id"=>"H0MA01024",
        "name"=>"ABAIR, PETER JON",
        "cycles"=>[2000]},
       {"candidate_status"=>"N",
        "active_through"=>2012,
        "incumbent_challenge_full"=>"Open seat",
        "district"=>"00",
        "election_years"=>[2012],
        "party"=>"DEM",
        "office"=>"H",
        "candidate_status_full"=>"Not yet a statutory candidate",
        "state"=>"MT",
        "incumbent_challenge"=>"O",
        "party_full"=>"Democratic Party",
        "office_full"=>"House",
        "candidate_id"=>"H2MT00039",
        "name"=>"ABARR, JOHN ALLEN",
        "cycles"=>[2012]},
       {"candidate_status"=>"C",
        "active_through"=>2008,
        "incumbent_challenge_full"=>"Unknown",
        "district"=>"05",
        "election_years"=>[2006, 2008],
        "party"=>"DEM",
        "office"=>"H",
        "candidate_status_full"=>"Statutory candidate",
        "state"=>"NJ",
        "incumbent_challenge"=>nil,
        "party_full"=>"Democratic Party",
        "office_full"=>"House",
        "candidate_id"=>"H6NJ05155",
        "name"=>"ABATE, CAMILLE M",
        "cycles"=>[2006, 2008]},
       {"candidate_status"=>"P",
        "active_through"=>1992,
        "incumbent_challenge_full"=>"Unknown",
        "district"=>"12",
        "election_years"=>[1992],
        "party"=>"DEM",
        "office"=>"H",
        "candidate_status_full"=>"Statutory candidate in a prior cycle",
        "state"=>"NJ",
        "incumbent_challenge"=>nil,
        "party_full"=>"Democratic Party",
        "office_full"=>"House",
        "candidate_id"=>"H2NJ12036",
        "name"=>"ABATE, FRANK G",
        "cycles"=>[1992]},
       {"candidate_status"=>"C",
        "active_through"=>1990,
        "incumbent_challenge_full"=>"Unknown",
        "district"=>"03",
        "election_years"=>[1990],
        "party"=>"REP",
        "office"=>"H",
        "candidate_status_full"=>"Statutory candidate",
        "state"=>"IA",
        "incumbent_challenge"=>nil,
        "party_full"=>"Republican Party",
        "office_full"=>"House",
        "candidate_id"=>"H0IA03071",
        "name"=>"ABBAS, JEFFREY LYN",
        "cycles"=>[1990]},
       {"candidate_status"=>"C",
        "active_through"=>1994,
        "incumbent_challenge_full"=>"Unknown",
        "district"=>"15",
        "election_years"=>[1994],
        "party"=>"REP",
        "office"=>"H",
        "candidate_status_full"=>"Statutory candidate",
        "state"=>"TX",
        "incumbent_challenge"=>nil,
        "party_full"=>"Republican Party",
        "office_full"=>"House",
        "candidate_id"=>"H4TX15043",
        "name"=>"ABBOTT, BONNIE",
        "cycles"=>[1994]},
       {"candidate_status"=>"N",
        "active_through"=>2016,
        "incumbent_challenge_full"=>"Open seat",
        "district"=>nil,
        "election_years"=>[2016],
        "party"=>"IND",
        "office"=>"P",
        "candidate_status_full"=>"Not yet a statutory candidate",
        "state"=>"US",
        "incumbent_challenge"=>"O",
        "party_full"=>"Independent",
        "office_full"=>"President",
        "candidate_id"=>"P60005840",
        "name"=>"ABBOTT, GIFFORD WHEELER JR",
        "cycles"=>[2016]},
       {"candidate_status"=>"P",
        "active_through"=>1996,
        "incumbent_challenge_full"=>"Challenger",
        "district"=>"00",
        "election_years"=>[1996],
        "party"=>"DEM",
        "office"=>"H",
        "candidate_status_full"=>"Statutory candidate in a prior cycle",
        "state"=>"SD",
        "incumbent_challenge"=>"C",
        "party_full"=>"Democratic Party",
        "office_full"=>"House",
        "candidate_id"=>"H6SD00077",
        "name"=>"ABBOTT, JAMES W",
        "cycles"=>[1996]},
       {"candidate_status"=>"P",
        "active_through"=>1992,
        "incumbent_challenge_full"=>"Unknown",
        "district"=>nil,
        "election_years"=>[1988, 1992],
        "party"=>"DEM",
        "office"=>"P",
        "candidate_status_full"=>"Statutory candidate in a prior cycle",
        "state"=>"US",
        "incumbent_challenge"=>nil,
        "party_full"=>"Democratic Party",
        "office_full"=>"President",
        "candidate_id"=>"P80002579",
        "name"=>"ABBOTT, JOHN HANCOCK",
        "cycles"=>[1992]},
       {"candidate_status"=>"N",
        "active_through"=>1988,
        "incumbent_challenge_full"=>"Unknown",
        "district"=>nil,
        "election_years"=>[1986, 1988],
        "party"=>"DEM",
        "office"=>"S",
        "candidate_status_full"=>"Not yet a statutory candidate",
        "state"=>"CA",
        "incumbent_challenge"=>nil,
        "party_full"=>"Democratic Party",
        "office_full"=>"Senate",
        "candidate_id"=>"S6CA00345",
        "name"=>"ABBOTT, JOHN HANCOCK",
        "cycles"=>[1986]},
       {"candidate_status"=>"T",
        "active_through"=>2004,
        "incumbent_challenge_full"=>"Unknown",
        "district"=>"18",
        "election_years"=>[2002, 2004],
        "party"=>"REP",
        "office"=>"H",
        "candidate_status_full"=>"Obsolete",
        "state"=>"TX",
        "incumbent_challenge"=>nil,
        "party_full"=>"Republican Party",
        "office_full"=>"House",
        "candidate_id"=>"H2TX18082",
        "name"=>"ABBOTT, PHILLIP J.",
        "cycles"=>[2002, 2004]},
       {"candidate_status"=>"P",
        "active_through"=>2000,
        "incumbent_challenge_full"=>"Unknown",
        "district"=>"03",
        "election_years"=>[1994, 1996, 2000],
        "party"=>"DEM",
        "office"=>"H",
        "candidate_status_full"=>"Statutory candidate in a prior cycle",
        "state"=>"KY",
        "incumbent_challenge"=>nil,
        "party_full"=>"Democratic Party",
        "office_full"=>"House",
        "candidate_id"=>"H4KY03095",
        "name"=>"ABBOTT, RAYMOND H",
        "cycles"=>[1994, 1996, 1998, 2000]},
       {"candidate_status"=>"N",
        "active_through"=>2014,
        "incumbent_challenge_full"=>"Challenger",
        "district"=>"03",
        "election_years"=>[2014],
        "party"=>"DEM",
        "office"=>"H",
        "candidate_status_full"=>"Not yet a statutory candidate",
        "state"=>"MI",
        "incumbent_challenge"=>"C",
        "party_full"=>"Democratic Party",
        "office_full"=>"House",
        "candidate_id"=>"H4MI03201",
        "name"=>"ABBOTT, RICHARD ALLEN IV",
        "cycles"=>[2014]},
       {"candidate_status"=>"P",
        "active_through"=>2004,
        "incumbent_challenge_full"=>"Unknown",
        "district"=>"04",
        "election_years"=>[2002, 2004],
        "party"=>"DEM",
        "office"=>"H",
        "candidate_status_full"=>"Statutory candidate in a prior cycle",
        "state"=>"IN",
        "incumbent_challenge"=>nil,
        "party_full"=>"Democratic Party",
        "office_full"=>"House",
        "candidate_id"=>"H2IN04101",
        "name"=>"ABBOTT, WILLIAM ANTHONY",
        "cycles"=>[2002, 2004]},
       {"candidate_status"=>"P",
        "active_through"=>1988,
        "incumbent_challenge_full"=>"Challenger",
        "district"=>"02",
        "election_years"=>[1988],
        "party"=>"REP",
        "office"=>"H",
        "candidate_status_full"=>"Statutory candidate in a prior cycle",
        "state"=>"NE",
        "incumbent_challenge"=>"C",
        "party_full"=>"Republican Party",
        "office_full"=>"House",
        "candidate_id"=>"H8NE02089",
        "name"=>"ABBOUD, CHRIS",
        "cycles"=>[1988]},
       {"candidate_status"=>"C",
        "active_through"=>2008,
        "incumbent_challenge_full"=>"Challenger",
        "district"=>"16",
        "election_years"=>[2008],
        "party"=>"DEM",
        "office"=>"H",
        "candidate_status_full"=>"Statutory candidate",
        "state"=>"IL",
        "incumbent_challenge"=>"C",
        "party_full"=>"Democratic Party",
        "office_full"=>"House",
        "candidate_id"=>"H8IL16062",
        "name"=>"ABBOUD, ROBERT",
        "cycles"=>[2008]}]}
  }

  describe '#candidates' do
    context 'without api_key' do
      it "returns an error" do
        candidates = OpenFecApi.candidates

        expect(candidates.code).to eql(403)
        expect(candidates.response).to be_kind_of(Net::HTTPForbidden)
        expect(candidates).to eql(api_key_missing_response)
      end
    end

    context 'with api_key' do
      it "returns an array of candidate objects" do
        OpenFecApi.config(api_key)
        candidates = OpenFecApi.candidates

        expect(candidates).to be_kind_of(Array)
        expect(candidates).not_to be_empty
        expect(candidates.first).to be_kind_of(Hash)
      end
    end
  end
end
